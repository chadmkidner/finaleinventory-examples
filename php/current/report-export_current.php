...
file_put_contents("finale-exp/fininv.csv",$result);
$tab = fopen("finale-exp/veilep_ftp_channelmax.txt","w");
$csv = fopen("finale-exp/fininv.csv","r");

fgetcsv($csv);
//$line[] = "SellingVenue";
//$line = array("SKU","Description","Category","Manufacturer","Std bin ID","Std packing","Units QoH","Units Reserved","Units Remaining","Units On order","Quantity","SellingVenue");
$tabLine = array("SKU","Quantity","SellingVenue");
fputcsv($tab,$tabLine,"\t");
while(!feof($csv))
{
	$line = fgetcsv($csv);
	//$line[] = "AmazonUS";
	$tabLine = array($line[0],$line[10],"AmazonUS");
	fputcsv($tab,$tabLine,"\t");
}
...
echo "Successfully exported report to file\n";
?>