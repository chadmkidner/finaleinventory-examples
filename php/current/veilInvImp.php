...
file_put_contents("fiver.csv",$result);

$con = mysqli_connect("localhost","aetherbi_jtest","J12345","aetherbi_fiver");
if (mysqli_connect_errno())
{
	echo "Failed to connect to MySQL: " . mysqli_connect_error();
}
$file = fopen("veilInvImp.csv","r");
fgetcsv($file);
$i = 0;
while(!feof($file))
{
	$line = fgetcsv($file);
	echo $i++.'   ';
	if(mysqli_query($con,"UPDATE uc_product_stock SET stock=".$line[10]." WHERE sku='".$line[0]."'") == false)
	{
		echo 'query failed   ';
	}
}
